...
def convert_btx_to_png(input_path, original_name=None):
    ...
    try:
        ...
        command = f'"{PVR_TOOL_PATH}" -i "{temp_ktx}" -o "{temp_pvr}" -d "{output_png}"'
...
def convert_png_to_btx(input_path):
    file_name = os.path.splitext(os.path.basename(input_path))[0]
    temp_ktx = os.path.join(TEMP_DIR, f"{file_name}_{os.urandom(4).hex()}.ktx")
    output_btx = os.path.join(TEMP_DIR, f"{file_name}.btx")

    try:
        command = f'"{PVR_TOOL_PATH}" -i "{input_path}" -o "{temp_ktx}" -f {PVR_TOOL_FORMAT}'
        result = subprocess.run(command, shell=True, capture_output=True, text=True)

        if result.returncode != 0:
            raise Exception(f"Ошибка PVRTexToolCLI: {result.stderr}")

        with open(temp_ktx, 'rb') as f:
            original_data = f.read()
        new_data = b'\x02\x00\x00\x00' + original_data
        with open(output_btx, 'wb') as f:
            f.write(new_data)
...
async def is_valid_file(file_path: str, file_extension: str) -> bool:
    try:
        if file_extension == '.png':
            try:
            ...
            try:
                with open(file_path, 'rb') as f:
                    header = f.read(4)
                    if header != b'\x02\x00\x00\x00':
                        return False
                return True
            except Exception as e:
                print(f"Invalid BTX file: {e}")
                return False
        elif file_extension == '.zip':
            try:
                with zipfile.ZipFile(file_path, 'r') as zip_ref:
                    # Просто проверяем, можно ли открыть архив
                    pass
                return True
            except Exception as e:
        ...
        print(f"File {file_path} is likely corrupted or has an invalid format: {e}")
        return False
